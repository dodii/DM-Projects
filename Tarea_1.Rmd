...
```{r}
# RESPUESTA
# Podemos crear una matriz cuyas componentes internas sean gráficos de correlación de a pares para todas las variables en relación al precio:           [codigo]

#También podemos hacer esto para el parámetro table:
#codigo
...
**Respuesta: Podrían explorarse agrupándose en cúmulos para sus precios según todos los criterios que afecten tal variable: el tamaño, la transparencia, el color, el pulido (que no está en la tabla, pero sirve para ejemplificar), etc. Con tales datos, se podría revisar qué tanto "pesa" cada criterio a la hora de fijar un precio, podría ser que el color de un diamante tenga una ponderación mayor en su valor monetario en comparación al tamaño, entre varios casos más.**